...
public abstract class Collector extends Character {
     ...
     * checks if the link is a trigger and if the bomb is not triggered if(true) then,
    ...
    protected void changeTile(Tile link) {
        if (!(link.getItem() instanceof Bomb)) {
            if ((link.getItem().equals(((Bomb) link.getItem()).getBombTriggerRight()) ||
                    link.equals(((Bomb) link.getItem()).getBombTriggerLeft()) ||
                    link.equals(((Bomb) link.getItem()).getBombTriggerDown()) ||
                    link.equals(((Bomb) link.getItem()).getBombTriggerUp()))) {
                ((Bomb) link.getItem()).trigger();
